...
missing_summary <- count_in_locations("AAAAAAA")
...
state_summary <- sapply(uniq_states, count_in_locations)
#                                         Note 10.
...
get_purposes <- function() {
  regular_pattern <- "\\s*\\([^\\)]+\\)"
  str_replace(purpose, regular_pattern, "")
}
